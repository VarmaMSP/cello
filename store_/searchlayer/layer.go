...
func NewSearchLayer(baseStore store_.Store, broker searchengine.Broker) store_.Store {
	return &searchStore{
		Store:   baseStore,
		podcast: &searchPodcastStore{PodcastStore: baseStore.Podcast(), podcastIndex: broker.Podcast()},
		episode: &searchEpisodeStore{EpisodeStore: baseStore.Episode(), episodeIndex: broker.Episode()},
	}
}
