...
export default class AudioPlayerSmall extends Component<Props, State> {
  ...
  render() {
    const { expand } = this.state
    const { 
      podcast, episode, albumArt,
      audioState, currentTime, duration,
      handleActionButtonPress, handleSeek,
      handleFastForward
    } = this.props
      ...
      <section className="player justify-between flex h-16 w-full px-2">
        ...
        <div className="flex-none flex items-center">
          ...
          <div style={{
            ...
            <ButtonWithIcon className="w-10 mx-1" icon="cheveron-up" onClick={this.handleExpandToggle}/>
          </div>
        </div>
      </section>

      {/* Player Maximised */}
      <section className="player-max flex flex-col justify-start w-full mb-6" style={{
        willChange: "height",
        height: expand ? "100%" : "0%",
        ...
        <section className="px-3 flex flex-row">
          <img className="h-32 w-32 flex-none object-cover object-center rounded" src={albumArt}/>
          <section className="flex-1 ml-3">
            <h4 className="text-md font-semibold text-green-600 leading-loose">Now Playing</h4>
            <h5 className="text-md font-bold text-gray-700 leading-tight">{episode}</h5>
            <h5 className="text-sm font-semibold text-gray-600 leading-loose">{podcast}</h5>
          </section>
        </section>
